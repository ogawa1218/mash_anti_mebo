#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""第 サムネイル生成スクリプト（HTML/CSS + Playwright）1280x670px"""
import sys
...
LINE2        = "メタボが加速させる"
LINE3        = "「老化」の正体。"
SUB          = "慢性炎症（Inflammaging）という共通原因"
NUM          = "\u2467"
OUTPUT_FNAME = "サムネイル_第14回.png"
...
.accent {{
  position:absolute; left:80px; top:72px;
  width:72px; height:6px; background:#FF6D00;
  border-radius:3px; box-shadow:0 0 18px rgba(255,109,0,.7);
}}
.copy {{ position:absolute; left:80px; top:108px; }}
.line1 {{
  display:block; font-size:86px; font-weight:900;
  color:#FFFFFF; line-height:1.18; letter-spacing:-2px;
}}
.line2 {{
  display:block; font-size:86px; font-weight:900;
  color:#FFFFFF; line-height:1.18; letter-spacing:-2px;
...
.line3 {{
  display:block; font-size:96px; font-weight:900;
  color:#FF6D00; line-height:1.2; margin-top:14px; letter-spacing:-2px;
  text-shadow: 0 0 50px rgba(255,109,0,.6), 0 0 100px rgba(255,109,0,.28);
}}
.sub {{
  display:block; font-size:36px; font-weight:500;
  color:#666666; line-height:1.4; margin-top:20px;
}}
.author {{
  position:absolute; bottom:26px; left:80px;
  font-size:22px; font-weight:400; color:#555555;
}}
.num {{
  position:absolute; bottom:16px; right:60px;
  font-size:80px; font-weight:900; color:#FF6D00;
